...
class FinancialAssumptions:
    scc_usd_per_ton: float = 190.0
    energy_price_usd_per_mwh: float = 72.0
    peak_demand_charge_usd_per_kw_year: float = 180.0
    discount_rate: float = 0.08
    analysis_horizon_years: int = 10
    carbon_price_low_usd_per_ton: float = 95.0
    carbon_price_high_usd_per_ton: float = 300.0
    extreme_threshold_q: float = 0.95
    contract_peak_mw: float = 40.0
    peak_breach_hours: float = 120.0
    peak_breach_penalty_usd_per_mwh: float = 450.0
    electricity_volatility_markup_pct: float = 0.05
    cooling_maintenance_scale_usd: float = 2_500_000.0
...
def main() -> None:
    ...
    _write_csv(run_dir / "monetary_numbers.csv", master_rows)
