...
def _sobol_indices(
    ...
    def sample_matrix() -> List[List[float]]:
        ...
        for _ in range(n):
            ...
            for name in names:
                lo, hi = ranges[name]
                row.append(lo + (hi - lo) * rng.random())
            m.append(row)
        return m

    def f(row: Sequence[float]) -> float:
        vals = {names[i]: row[i] for i in range(len(names))}
        out = _physics(
    ...
    fB = [f(r) for r in B]
    y_all = fA + fB
    mean_y = statistics.fmean(y_all)
    var_y = statistics.fmean([(x - mean_y) ** 2 for x in y_all])
    ...
    for i, name in enumerate(names):
        ...
        for a, b in zip(A, B):
            ...
            mixed[i] = b[i]
            ABi.append(mixed)
        fAB = [f(r) for r in ABi]
        s1_raw = statistics.fmean([fB[j] * (fAB[j] - fA[j]) for j in range(n)]) / var_y
        st_raw = 0.5 * statistics.fmean([(fA[j] - fAB[j]) ** 2 for j in range(n)]) / var_y
        s1 = _clip(s1_raw, 0.0, 1.0)
        st = _clip(st_raw, 0.0, 1.0)
        rows.append({"parameter": name, "S1": s1, "ST": st, "output_metric": output_key})
    return rows
...
def _copula_analysis(merged: List[Dict[str, float]]) -> Tuple[List[Dict[str, float]], List[Dict[str, float]], Dict[str, List[float]]]:
    ...
    for name, (u, v) in pair_map.items():
        ...
        summary_rows.append(
            {"pair": name, "upper_tail_q95": lu, "lower_tail_q05": ll}
        )
...
def _build_energy_forecast_rows(
    ...
    rows: List[Dict[str, float]] = []
    for scenario, g in growth.items():
        for i, yr in enumerate(years):
            power_mw = base_power_mw * ((1.0 + g) ** i)
            peak_mw = base_peak_mw * ((1.0 + 0.55 * g) ** i)
            annual_energy_gwh = power_mw * 8760.0 / 1000.0
            rows.append(
                {
                    "forecast_scenario": scenario,
                    "growth_rate": g,
                    "year": yr,
                    "forecast_total_power_mw": power_mw,
                    "forecast_peak_mw": peak_mw,
                    "forecast_annual_energy_gwh": annual_energy_gwh,
                    "forecast_annual_energy_mwh": annual_energy_gwh * 1000.0,
                }
            )
    return rows
...
def _render_sobol_dual(
    ...
    panels = [
        {"x": 90, "y": 130, "w": 820, "h": 760, "title": "Standalone Variable Effects", "subtitle": "(Direct contribution to variance)", "vals": s1, "label": "First-Order Sobol Index S_i", "color": "#4c84b1"},
        {"x": 980, "y": 130, "w": 820, "h": 760, "title": "Total Variable Effects", "subtitle": "(Including interaction terms)", "vals": st, "label": "Total-Order Sobol Index S_Ti", "color": "#f67c4b"},
    ]

    for p in panels:
        px, py, pw, ph = p["x"], p["y"], p["w"], p["h"]
        vals = p["vals"]
        vmax = max(max(vals), 1e-9)
        n = len(vals)
        row_h = ph / max(n, 1)
...
def _render_energy_peak_projection(
    ...
    vals = annual_gwh.get("Moderate 15%", next(iter(annual_gwh.values())))
    vmax = max(vals) * 1.12
    n = len(years)
    ...
    legend_order = [k for k in ["Conservative 5%", "Moderate 15%", "Aggressive 30%"] if k in peak_mw]
    if not legend_order:
...
def main() -> None:
    ...
    for r in sobol_top:
        report_lines.append(f"- `{r['parameter']}`: S1={float(r['S1']):.4f}, ST={float(r['ST']):.4f}")
