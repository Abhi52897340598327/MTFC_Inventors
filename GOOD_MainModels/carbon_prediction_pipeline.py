...
from random_forest_model import fit as fit_power_model  # same RF architecture for Stage 4
from random_forest_model import predict as predict_power_model
from schema import (
    RunArtifacts,
    StageMetrics,
...
class CarbonForecastPipeline:
    ...
    def _build_supervised_views(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        dfx, pw_features, pw_lag_map, pw_allow_zero = self._build_power_features(dfx)
        ...
        # Stage 4 power view (only when power data available)
        pw_view = pd.DataFrame()
        if pw_features:
            assert_strictly_past_features(pw_lag_map, allow_zero_for=pw_allow_zero)
            pw_cols = ["timestamp", "observed_total_power_mw_t_plus_h"] + pw_features
            pw_cols = [c for c in pw_cols if c in dfx.columns]
            if "observed_total_power_mw_t_plus_h" in dfx.columns:
                pw_view = dfx[pw_cols].dropna(subset=pw_features + ["observed_total_power_mw_t_plus_h"]).copy()
            self.feature_cols["stage4_power"] = pw_features
            self.feature_lag_map["stage4_power"] = pw_lag_map
            self.zero_lag_allow["stage4_power"] = pw_allow_zero
        ...
        return cpu_view, ci_view, pw_view, merged_targets
    ...
    def fit(self, df_train: pd.DataFrame) -> "CarbonForecastPipeline":
        ...
        cpu_view, ci_view, pw_view, _ = self._build_supervised_views(df_train)

        has_power_model = (
            not pw_view.empty
            and "stage4_power" in self.feature_cols
            and len(self.feature_cols["stage4_power"]) > 0
        )
        self.log.info(
            "Training samples: Stage1=%s Stage4(power)=%s Stage5=%s",
            len(cpu_view),
            len(pw_view) if has_power_model else "N/A",
            len(ci_view),
        )
        ...
        all_cv_dfs = [cpu_cv_df, ci_cv_df]

        # Stage 4: direct ML power model (when observed power data is available)
        if has_power_model and len(pw_view) >= (self.config.cv_splits + 1) * 20:
            pw_model, pw_params, pw_cv_df, pw_oof = self._select_params_and_train(
                pw_view,
                self.feature_cols["stage4_power"],
                "observed_total_power_mw_t_plus_h",
                "stage4_power",
                self.config.cpu_param_grid,  # same RF grid
                fit_power_model,
                predict_power_model,
            )
            self.models["stage4_power"] = pw_model
            self.best_params["stage4_power"] = pw_params
            all_cv_dfs.append(pw_cv_df)
            pw_oof_mask = np.isfinite(pw_oof)
            if pw_oof_mask.any():
                pw_oof_metrics = calc_metrics(
                    pw_view.loc[pw_oof_mask, "observed_total_power_mw_t_plus_h"],
                    pw_oof[pw_oof_mask],
                )
                self.log.info("Stage4 OOF RMSE=%.4f R2=%.6f", pw_oof_metrics["RMSE"], pw_oof_metrics["R2"])

        self.cv_metrics_df = pd.concat(all_cv_dfs, ignore_index=True)
    ...
    def _prepare_eval_views(self, df_holdout: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        cpu_view, ci_view, pw_eval, _ = self._build_supervised_views(combined)
        self._eval_pw_view = pw_eval  # stash for evaluate() to access
    ...
    def evaluate(
        ...
        # Stage 4: use ML power model when available — replaces physics-based total power
        if "stage4_power" in self.models and hasattr(self, "_eval_pw_view") and self._eval_pw_view is not None and not self._eval_pw_view.empty:
            holdout_ts = set(merged["timestamp"])
            pw_eval_rows = self._eval_pw_view[self._eval_pw_view["timestamp"].isin(holdout_ts)].copy()
            if not pw_eval_rows.empty and len(pw_eval_rows) >= 10:
                feat_cols = self.feature_cols.get("stage4_power", [])
                available_feats = [c for c in feat_cols if c in pw_eval_rows.columns]
                if available_feats:
                    X_pw = pw_eval_rows[available_feats].to_numpy(dtype=float)
                    ml_total = predict_power_model(self.models["stage4_power"], X_pw)
                    ml_total = np.clip(ml_total, 0.0, self.config.facility_mw * self.config.max_pue)
                    pw_ts_map = dict(zip(pw_eval_rows["timestamp"], ml_total))
                    merged_ml_power = merged["timestamp"].map(pw_ts_map)
                    fill_mask = merged_ml_power.notna()
                    if fill_mask.sum() >= 10:
                        merged["pred_total_power"] = cal_total  # default physics fallback
                        merged.loc[fill_mask, "pred_total_power"] = merged_ml_power[fill_mask].to_numpy(dtype=float)
                        n_ml = int(fill_mask.sum())
                        self.log.info("Stage4 ML power: %d/%d rows from ML model (%.0f%%)", n_ml, len(merged), 100*n_ml/len(merged))
                    else:
...
def run_validate_inputs(pipeline: CarbonForecastPipeline) -> None:
    data = pipeline.load_data()
    cpu_view, ci_view, _, _ = pipeline._build_supervised_views(data)
