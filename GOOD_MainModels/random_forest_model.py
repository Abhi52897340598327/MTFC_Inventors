...
def predict_quantile_intervals(
    ...
    """Compute per-sample quantile prediction intervals from individual tree predictions.

    Each tree in a RandomForest / ExtraTrees ensemble produces one prediction.
    Taking percentiles across trees gives a natural, non-parametric interval.

    Parameters
    ----------
    model : RandomForestRegressor or ExtraTreesRegressor
    ...
    # Stack per-tree predictions: shape (n_estimators, n_samples)
...
__all__ = ["build_model", "fit", "predict", "predict_quantile_intervals", "evaluate", "DEFAULT_PARAMS"]
