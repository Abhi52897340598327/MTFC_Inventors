        ...
        for yr in years:
            ...
            if sarimax_model:
                ...
                try:
                    ...
                    # Use only causal-safe fill rules (no backward fill).
                    pred_hourly = pred_weekly.reindex(df_fut.set_index("timestamp").index).interpolate(method="linear")
                    pred_hourly = pred_hourly.ffill()
                    if pred_hourly.isna().any():
                        fallback = float(pred_weekly.iloc[0]) if len(pred_weekly) else float(df_fut["grid_demand_mw_xgb"].iloc[0])
                        pred_hourly = pred_hourly.fillna(fallback)
