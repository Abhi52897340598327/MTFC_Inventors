...
This script generates 7 time series line charts from the combined and cleaned
electricity load dataset and saves them to the Figures-isaac folder.
...
# Use a simple sans-serif font and disable TeX rendering to avoid renderer hangs
...
# ============================================================================
# 4. CHART 2: FIRST 30 DAYS DETAIL VIEW
# ============================================================================
...
# ============================================================================
# 5. CHART 3: WEEKLY AVERAGES
# ============================================================================
df_copy = df.copy()
df_copy['year'] = df_copy['datetime_beginning_utc'].dt.isocalendar().year
df_copy['week'] = df_copy['datetime_beginning_utc'].dt.isocalendar().week
weekly_avg = df_copy.groupby(['year', 'week'])['mw'].mean().reset_index()

# Convert ISO week to datetime properly to avoid invalid week 53 errors
def iso_to_date(year, week):
    ...
    try:
        ...
        # If week 53 doesn't exist, use week 52
        return pd.to_datetime(f'{year}-W52-1', format='%G-W%V-%u')


weekly_avg['date'] = weekly_avg.apply(lambda row: iso_to_date(row['year'], row['week']), axis=1)
weekly_avg['date'] = pd.to_datetime(weekly_avg['date'])

fig, ax = plt.subplots(figsize=(14, 6))
ax.plot(weekly_avg['date'], weekly_avg['mw'], linewidth=2, color='#2ca02c', marker='o', markersize=5)
...
# ============================================================================
# 6. CHART 4: DAILY AVERAGES WITH MIN/MAX RANGE
# ============================================================================
daily_avg = df.groupby(df['datetime_beginning_utc'].dt.date)['mw'].agg(['mean', 'min', 'max']).reset_index()
daily_avg['datetime_beginning_utc'] = pd.to_datetime(daily_avg['datetime_beginning_utc'])

fig, ax = plt.subplots(figsize=(14, 6))
ax.plot(daily_avg['datetime_beginning_utc'], daily_avg['mean'], 
        linewidth=1.5, color='#d62728', label='Daily Average', marker='o', markersize=3)
ax.fill_between(daily_avg['datetime_beginning_utc'], daily_avg['min'], daily_avg['max'],
...
# ============================================================================
# 7. CHART 5: HOURLY PATTERN (AVERAGE LOAD BY HOUR OF DAY)
# ============================================================================
...
# ============================================================================
# 9. CHART 7: LOAD DISTRIBUTION AND VARIABILITY
# ============================================================================
